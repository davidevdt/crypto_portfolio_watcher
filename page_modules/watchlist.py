"""
Watchlist Page - Identical to Monitoring Page but for watchlist assets
"""
    ...
    try:
        ...
        if historical_prices and len(historical_prices) >= 10:
            ...
            if interval == "4h":
                ...
                # Use data as-is for other intervals
                agg_df = df
...
def show():
    ...
    show_detailed_asset_info(watchlist_interval, watchlist_days)
            ...
            if st.button("💾 Save Monitoring Parameters"):
                ...
                if hasattr(st.session_state, "historical_data"):
                    ...
                    timestamp_keys_to_remove = [
                        ...
                        if "_watchlist_timestamp" in key
                    ]
...
def show_watchlist_monitoring():
    """Show enhanced watchlist monitoring with new signal types. Returns timeframe parameters."""
    col1, col2 = st.columns([3, 1])
    with col1:
        st.subheader("📊 Watchlist Monitoring")
    with col2:
    ...
    with col1:
        _, interval, days = universal_timeframe_selector(
            "watchlist",
    ...
    watchlist_interval = interval
    watchlist_days = days

    # Get watchlist assets for monitoring
    try:
        watchlist = st.session_state.portfolio_manager.get_watchlist()
    except Exception as e:
        st.error(f"Error loading watchlist: {e}")
        st.info("Please try refreshing the page or check your database connection.")
    ...
    unique_symbols = {}
    for item in watchlist:
        if item.symbol not in ["USDT", "USDC", "BUSD"]:  # Exclude stablecoins
            unique_symbols[item.symbol] = item

    if not unique_symbols:
        show_empty_state(
            title="No Non-Stablecoin Assets",
            message="Your watchlist only contains stablecoins. Add some cryptocurrencies like BTC, ETH, etc. to monitor their price movements and signals.",
            icon="👀",
        )
        return watchlist_interval, watchlist_days
    ...
    with st.spinner("Fetching data for analysis..."):
        for symbol in symbols_list:
            ...
            cache_key = f"{symbol}_{watchlist_interval}_{watchlist_days}_watchlist"
            cache_timestamp_key = (
                f"{symbol}_{watchlist_interval}_{watchlist_days}_watchlist_timestamp"
            )
            ...
            if (
                ...
                recent_data = get_extended_historical_data(
                    symbol, watchlist_days, watchlist_interval
                )
                if recent_data:
    ...
    if monitoring_data:
        ...
        st.markdown("### 📈 Watchlist Monitoring Dashboard")

        # Create simple styled dataframe like Portfolio Assets table
        create_monitoring_dataframe(df_monitoring, watchlist_interval)

    else:
        st.error(
            "Unable to analyze any watchlist assets. Please check your data connections."
        )
...
def analyze_asset_signals(df: pd.DataFrame, symbol: str) -> Dict:
    ...
    try:
        ...
        _ = df["high"].values  # highs not used in current analysis
        _ = df["low"].values  # lows not used in current analysis
        _ = df["volume"].values if "volume" in df.columns else None  # volumes not used
        ...
        vol_regime, _, vol_emoji = get_volatility_indicator(vol_ratio)
...
def create_monitoring_dataframe(df: pd.DataFrame, _: str = "1d"):
    """Create a simple styled dataframe like Portfolio Assets table."""

    # Get configurable parameters from settings (unused in current implementation)
    _ = st.session_state.settings.get("volatility_low_threshold", 0.7)
    _ = st.session_state.settings.get("volatility_high_threshold", 1.3)
    _ = st.session_state.settings.get("rsi_periods", 14)
    _ = st.session_state.settings.get("sma_trend_periods", 20)
    _ = st.session_state.settings.get("sma_medium_periods", 50)
    _ = st.session_state.settings.get("sma_long_periods", 200)
    _ = st.session_state.settings.get("ema_trend_periods", 20)
    _ = st.session_state.settings.get("ema_medium_periods", 50)
    ...
    for _, row in df.iterrows():
        ...
        _ = row["Volatility_Ratio"]  # vol_ratio not used in display
...
def show_detailed_asset_info(watchlist_interval: str, watchlist_days: int):
            ...
            with st.spinner("Calculating detailed indicators..."):
                for symbol in selected_assets:
                    try:
                        ...
                        cache_key = (
                            f"{symbol}_{watchlist_interval}_{watchlist_days}_watchlist"
                        )
                        cache_timestamp_key = f"{symbol}_{watchlist_interval}_{watchlist_days}_watchlist_timestamp"
...
def check_all_signals():
    """Check signals for all watchlist assets and generate alerts."""
    try:
        ...
        with st.spinner("Checking signals for all watchlist assets..."):
            for item in watchlist:
                try:
                    if item.symbol not in ["USDT", "USDC", "BUSD"]:
                        # Get historical data using database cache first
                        historical_data = get_extended_historical_data(
                            item.symbol, 7, "1h"
                        )

                        if historical_data and len(historical_data) > 50:
                            df = pd.DataFrame(historical_data)
                            analysis = analyze_asset_signals(df, item.symbol)

                            # Generate alerts for significant RSI signals
                            rsi = analysis["rsi"]

                            if not np.isnan(rsi):
                                if rsi >= 70:  # Overbought
                                    signal_type = "OVERBOUGHT"
                                    strength = f"{((rsi - 70) / 30 * 100):.0f}%"
                                elif rsi <= 30:  # Oversold
                                    signal_type = "OVERSOLD"
                                    strength = f"{((30 - rsi) / 30 * 100):.0f}%"
                                else:
                                    continue  # No significant signal

                                strength_value = float(strength.replace("%", ""))
                                if strength_value >= 50:  # Only alert on strong signals
                                    alert = {
                                        "timestamp": datetime.now(),
                                        "symbol": item.symbol,
                                        "type": "WATCHLIST_SIGNAL",
                                        "message": f"Watchlist: {signal_type} signal detected (Strength: {strength})",
                                        "active": True,
                                        "severity": (
                                            "HIGH" if strength_value >= 75 else "MEDIUM"
                                        ),
                                        "signal": signal_type,
                                        "strength": strength,
                                    }
                                    ...
                                    if st.session_state.settings.get(
                                        ...
                                        send_desktop_notification(
                                            f"Watchlist Signal: {item.symbol}",
                                            f"{signal_type} - {strength} strength",
                                        )
                                    ...
                                    if st.session_state.settings.get(
                                        ...
                                        if phone:
                                            send_whatsapp_notification(
                                                f"🚨 Watchlist: {item.symbol} {signal_type} signal ({strength} strength)",
                                                phone,
                                            )
                            ...
                            if (
                                ...
                                alert = {
                                    ...
                                    "message": f"Watchlist: High volatility detected (Ratio: {vol_ratio:.2f})",
                                    "active": True,
                                    "severity": "MEDIUM",
                                    "volatility_ratio": vol_ratio,
                                }
                    ...
                    st.warning(f"Error checking signals for {item.symbol}: {e}")
