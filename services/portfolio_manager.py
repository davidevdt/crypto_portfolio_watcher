...
class PortfolioManager:
    ...
    def update_portfolio_name(self, portfolio_id: int, new_name: str):
        ...
        try:
            ...
            return {"id": portfolio.id, "name": new_name, "old_name": old_name}
        except Exception as e:
            session.rollback()
    ...
    def delete_portfolio(
        self, portfolio_id: int, move_assets_to_portfolio_id: Optional[int] = None
    ):
        """
        ...
        try:
            ...
            assets = (
                session.query(Asset).filter(Asset.portfolio_id == portfolio_id).all()
            )
            asset_count = len(assets)
            ...
            if move_assets_to_portfolio_id:
                ...
                target_portfolio = session.query(Portfolio).get(
                    move_assets_to_portfolio_id
                )
                if not target_portfolio:
                    raise ValueError(
                        f"Target portfolio with ID {move_assets_to_portfolio_id} not found"
                    )
                ...
                logger.info(
                    f"Moved {asset_count} assets from '{portfolio_name}' to '{target_portfolio.name}'"
                )
            else:
                ...
                for asset in assets:
                    session.delete(asset)
                logger.info(
                    f"Deleted {asset_count} assets with portfolio '{portfolio_name}'"
                )
            ...
            history_records = (
                ...
                .filter(PortfolioValueHistory.portfolio_id == portfolio_id)
                .all()
            )
            ...
            if history_records:
                logger.info(
                    f"Deleted {len(history_records)} portfolio value history records for '{portfolio_name}'"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Could not sync tracked assets after portfolio deletion: {e}"
                )
            ...
            logger.info(
                f"Successfully deleted portfolio '{portfolio_name}' with {asset_count} assets"
            )
            return result
